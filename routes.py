...
from flask import render_template, url_for, flash, redirect, request, jsonify, abort, make_response, Blueprint
...
from forms import (RegistrationForm, LoginForm, CategoryForm, SupplierForm, ProductForm, 
                  ...
                  SimpleCustomerForm)
from utils import (generate_invoice_number, generate_invoice_pdf, generate_unique_barcode, 
                  generate_barcode_image_for_invoice)
...
def edit_product(product_id):
    ...
    categories = Category.query.all()
    suppliers = Supplier.query.all()
    barcode_number = generate_unique_barcode()  # Safe to call here
    return render_template('products/add_product.html', categories=categories, suppliers=suppliers, barcode=barcode_number)


    # Load choices for category and supplier
...
def new_product():
    ...
    if form.validate_on_submit():
        new_product = Product(
            ...
            barcode=generate_unique_barcode()  # Set the generated barcode
        )
        db.session.add(new_product)
        db.session.commit()
        flash('Product added successfully!', 'success')
        return redirect(url_for('inventory'))
    return render_template('inventory.html', product_form=form)
